...
pub fn reward(fee: u64, height: u64) -> u64 {
	...
	let block_reward = calc_why_block_reward(height);
...
pub fn calc_why_block_reward(height: u64) -> u64 {
	if height == 0 {
		// Genesis block
...
pub fn calc_why_block_overage(height: u64, genesis_had_reward: bool) -> u64 {
...
mod test {
	...
	// MWC  testing calc_why_block_reward output for the scedule that documented at definition of calc_why_block_reward
	#[test]
	fn test_calc_why_block_reward() {
		global::set_local_chain_type(global::ChainTypes::Mainnet);

		// first blocks
		assert_eq!(calc_why_block_reward(1), 2_380_952_380);
		assert_eq!(calc_why_block_reward(2), 2_380_952_380);

		// a little deeper
		assert_eq!(calc_why_block_reward(100000), 2_380_952_380);

		// pre hard fork block
		assert_eq!(
			calc_why_block_reward(get_c31_hard_fork_block_height() - 1),
			2_380_952_380
		);
		assert_eq!(
			calc_why_block_reward(get_c31_hard_fork_block_height()),
			2_380_952_380
		);

		assert_eq!(
			calc_why_block_reward(get_c31_hard_fork_block_height() + WEEK_HEIGHT - 1),
			2_380_952_380
		);

		// reward changes 1 week after HF
		assert_eq!(
			calc_why_block_reward(get_c31_hard_fork_block_height() + WEEK_HEIGHT),
			600_000_000
		);

		// check epoch 2
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(2)),
			600_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(2) - 1),
			2_380_952_380
		);

		// check epoch 3
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(3)),
			450_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(3) - 1),
			600_000_000
		);

		// check epoch 4
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(4)),
			300_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(4) - 1),
			450_000_000
		);

		// check epoch 5
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(5)),
			250_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(5) - 1),
			300_000_000
		);

		// check epoch 6
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(6)),
			200_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(6) - 1),
			250_000_000
		);

		// check epoch 7
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(7)),
			150_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(7) - 1),
			200_000_000
		);

		// check epoch 8
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(8)),
			100_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(8) - 1),
			150_000_000
		);

		// check epoch 9
		assert_eq!(calc_why_block_reward(get_epoch_block_offset(9)), 50_000_000);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(9) - 1),
			100_000_000
		);

		// check epoch 10
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(10)),
			25_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(10) - 1),
			50_000_000
		);

		// check epoch 11
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(11)),
			10_000_000
		);

		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(11) - 1),
			25_000_000
		);

		// last block reward is special
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(11) + get_epoch_duration(11)),
			MWC_LAST_BLOCK_REWARD
		);

		// 0
		assert_eq!(
			calc_why_block_reward(get_epoch_block_offset(11) + get_epoch_duration(11) + 1),
			0
		);

		// far far future
		assert_eq!(calc_why_block_reward(2_100_000 * 320000000 + 200), 0); // no reward
	}

	// MWC  testing calc_why_block_overage output for the schedule that documented at definition of calc_why_block_reward
	#[test]
	fn test_calc_why_block_overage() {
		global::set_local_chain_type(global::ChainTypes::Mainnet);

		let genesis_reward: u64 = GENESIS_BLOCK_REWARD;

		assert_eq!(calc_why_block_overage(0, true), genesis_reward); // Doesn't make sense to call for the genesis block
		assert_eq!(calc_why_block_overage(0, false), 0); // Doesn't make sense to call for the genesis block
		assert_eq!(
			calc_why_block_overage(1, true),
			genesis_reward + MWC_FIRST_GROUP_REWARD * 1
		);

		assert_eq!(
			calc_why_block_overage(get_c31_hard_fork_block_height(), true),
			genesis_reward + MWC_FIRST_GROUP_REWARD * get_c31_hard_fork_block_height()
		);

		assert_eq!(
			calc_why_block_overage(get_c31_hard_fork_block_height() + WEEK_HEIGHT - 1, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_c31_hard_fork_block_height() + WEEK_HEIGHT - 1)
		);

		assert_eq!(
			calc_why_block_overage(get_c31_hard_fork_block_height() + WEEK_HEIGHT, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_c31_hard_fork_block_height() + WEEK_HEIGHT - 1)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(2), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_c31_hard_fork_block_height() + WEEK_HEIGHT - 1)
				+ get_epoch_reward(2) * 1
		);
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(3) - 1, true),
			genesis_reward
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(3), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(4), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(4) + 3, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(5), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(5) + 3, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(6), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(6) + 3, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(7), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(7) + 3, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(8), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(8) + 3, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(9), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(9) + 39, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(10), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(10) + 1, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(11), true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		assert_eq!(
			calc_why_block_overage(get_epoch_block_offset(11) + 39, true),
			genesis_reward
				+ MWC_FIRST_GROUP_REWARD * (get_epoch_block_offset(2) - 1)
				+ (get_epoch_block_offset(3) - get_epoch_block_offset(2)) * get_epoch_reward(2)
				+ (get_epoch_block_offset(4) - get_epoch_block_offset(3)) * get_epoch_reward(3)
		...
		let total_blocks_reward = calc_why_block_overage(2_100_000_000 * 1000, true);
