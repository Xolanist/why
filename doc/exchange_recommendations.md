...
The whyproject repository has two main wallets that are possible for exchanges to use:

why-wallet: https://github.com/whyproject/why-wallet

and why713: why713: https://github.com/whyproject/why713

The main difference between these wallets is that why713 supports receive by http(s), file, and whymqs, and keybase, while why-wallet supports http(s), file, and keybase. why-wallet is a fork of the grin-wallet so it is much closer to grin-wallet.

Exchanges that already support grin-wallet may have an easier time supporting why-wallet than why713. The latest version of the wallet may have bug fixes so please keep up to date on changes.
...
That is part of how it scales so much more easily than Bitcoin, but it means that wallet software and systems around that wallet software need to handle reorgs differently. The wallet has a separate state from the network. In the latest version of why-wallet and why713 most commands call scan and we attempt to keep the state of the wallet in exactly the same state as full node it is connected to.
...
```$ curl https://why713.why.mw/v1/chain/outputs/byids?id=<id> ```
...
This command should only be run after sufficient confirmations are obtained. In order to find the output for a deposit, this command can be used in why-wallet:

```why-wallet txs -i <tx_index>```
...
```why-wallet send --min_conf 5100 -d <destination> <amount>```
...
In addtion to this setup, it is possible to use some new settings a long with --min_conf in a single wallet to prevent double spends. See information about the new parameters here: https://github.com/whyproject/why-wallet/blob/master/doc/change_outputs.md
